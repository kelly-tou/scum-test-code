...
fixed_point_t lr_exp(const fixed_point_t y) {
...
bool lr_ln(const fixed_point_t x, fixed_point_t* result) {
    ...
    while (fixed_point_absolute_value(fixed_point_multiply(
        ...
        term = fixed_point_subtract(fixed_point_divide(x, lr_exp(y)),
                                    fixed_point_init(1));
...
bool linear_regression_init(const fixed_point_t signal_sampling_period,
    ...
    for (int i = 0; i < length; ++i) {
        ...
        if (!lr_ln(fixed_point_divide(fixed_point_init(data[i]) - minimum_value,
                                      amplitude),
                   &b_entry)) {
            return false;
        }
