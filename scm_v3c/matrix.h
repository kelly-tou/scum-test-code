...
// Matrix element type.
typedef fixed_point_short_t matrix_type_t;
...
typedef struct {
    ...
    // Matrix buffer.
    fixed_point_short_t buffer[MATRIX_MAX_SIZE];
} matrix_t;
...
bool matrix_set(matrix_t* matrix, size_t row, size_t col,
                fixed_point_short_t value);
...
// // Set the matrix element at the specified row and column to the given value.
// // Return whether the matrix element was successfully set.
// bool matrix_set(matrix_t* matrix, size_t row, size_t col, matrix_type_t
// value);
