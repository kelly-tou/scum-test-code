...
bool matrix_init(matrix_t* matrix, const size_t rows, const size_t cols) {
    ...
    memset(matrix->buffer, 0, MATRIX_MAX_SIZE * sizeof(fixed_point_short_t));
    return true;
}
...
bool matrix_set(matrix_t* matrix, const size_t row, const size_t col,
                const fixed_point_short_t value) {
    if (!matrix_validate(matrix, row, col)) {
        return false;
    }
...
bool matrix_copy(const matrix_t* matrix, matrix_t* result) {
    ...
    memcpy(result->buffer, matrix->buffer, size * sizeof(fixed_point_short_t));
    return true;
}
